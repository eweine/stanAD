  ...
  stan::math::gradient(
    [&Zty, &Xty, &Z, &Z2, &blocks_per_ranef, &terms_per_block, &n_ranef_par, &n_fixef_par, &X](auto par) {
      ...
      for (int k = 0; k < terms_per_block.size(); k++) {
        ...
        Eigen::Matrix<stan::math::var, Eigen::Dynamic, Eigen::Dynamic> L(terms_per_block[k], terms_per_block[k]);
        L.setZero();
        ...
        Eigen::Matrix<stan::math::var, Eigen::Dynamic, Eigen::Dynamic> Sigma = L * L.transpose();
        elbo -= 0.5 * static_cast<double>(blocks_per_ranef[k]) *
          stan::math::log_determinant(Sigma);

        Eigen::Matrix<stan::math::var, Eigen::Dynamic, Eigen::Dynamic> Sigma_inv = stan::math::inverse(Sigma);
