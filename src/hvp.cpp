...
Eigen::VectorXd pois_glmm_mfvb_hvp(
  ...
  // (m1, log_s1, m2, log_s2, ..., b1, ..., bp, 1/sigma1^2, ..., 1/sigmak^2)
  ...
  stan::math::hessian_times_vector(
    [&Zty, &Xty, &Z, &Z2, &blocks_per_ranef, &n_ranef_par, &n_fixef_par, &X](auto par) {
      ...
      Eigen::Matrix<stan::math::fvar<stan::math::var>, Eigen::Dynamic, 2> ranef_mat = par.segment(
        0, 2 * n_ranef_par
      ).reshaped(2, n_ranef_par).transpose();

      Eigen::Vector<stan::math::fvar<stan::math::var>, Eigen::Dynamic> s2 = stan::math::square(
        stan::math::exp(
          ranef_mat.col(1)
        )
      );

      stan::math::fvar<stan::math::var> elbo = stan::math::dot_product(Zty, ranef_mat.col(0)) +
